package com.wixct.blogapi.web.relationships;


import com.wixct.blogapi.web.util.DateUtil;
import com.wixct.blogapi.jfinal.model.Relationships;
...
public class RelationshipsController extends Controller {
	...
	*/
	private static final Logger logger = LoggerFactory.getLogger(RelationshipsController.class.getName());



	public void index() {
		render("/page/relationships/relationshipsList.html");
	}

	@ApiOperation("GITHUB项目列表")
	@RequestMapping(value="/relationships/show",method= RequestMethod.GET)
	@ResponseBody
	public Object show(//@ApiParam("项目名称") @RequestParam(value="pjname",defaultValue="",required=false) String pjname,
		...
		sql_exp.append("from t_relationships a where 1=1 ");
...
//		sql_exp.append("order by a.UPDATE desc");
		...
		Page<Relationships> pages= Relationships.dao.paginate(pageNum,pageSize,sql_select.toString(),sql_exp.toString(),params.toArray());
