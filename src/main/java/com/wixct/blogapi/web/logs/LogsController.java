package com.wixct.blogapi.web.logs;


import com.wixct.blogapi.web.util.DateUtil;
import com.wixct.blogapi.jfinal.model.Logs;
...
public class LogsController extends Controller {
	...
	*/
	private static final Logger logger = LoggerFactory.getLogger(LogsController.class.getName());



	public void index() {
		render("/page/logs/logsList.html");
	}

	@ApiOperation("GITHUB项目列表")
	@RequestMapping(value="/logs/show",method= RequestMethod.GET)
	@ResponseBody
	public Object show(//@ApiParam("项目名称") @RequestParam(value="pjname",defaultValue="",required=false) String pjname,
		...
		sql_exp.append("from t_logs a where 1=1 ");
...
//		sql_exp.append("order by a.UPDATE desc");
		...
		Page<Logs> pages= Logs.dao.paginate(pageNum,pageSize,sql_select.toString(),sql_exp.toString(),params.toArray());
