package com.wixct.blogapi.web.attach;


import com.wixct.blogapi.web.util.DateUtil;
import com.wixct.blogapi.jfinal.model.Attach;
...
public class AttachController extends Controller {
	...
	*/
	private static final Logger logger = LoggerFactory.getLogger(AttachController.class.getName());



	public void index() {
		render("/page/attach/attachList.html");
	}

	@ApiOperation("GITHUB项目列表")
	@RequestMapping(value="/attach/show",method= RequestMethod.GET)
	@ResponseBody
	public Object show(//@ApiParam("项目名称") @RequestParam(value="pjname",defaultValue="",required=false) String pjname,
		...
		sql_exp.append("from t_attach a where 1=1 ");
...
//		sql_exp.append("order by a.UPDATE desc");
		...
		Page<Attach> pages= Attach.dao.paginate(pageNum,pageSize,sql_select.toString(),sql_exp.toString(),params.toArray());
