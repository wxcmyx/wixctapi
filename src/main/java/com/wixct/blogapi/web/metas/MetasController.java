package com.wixct.blogapi.web.metas;


import com.wixct.blogapi.web.util.DateUtil;
import com.wixct.blogapi.jfinal.model.Metas;
...
@Api(tags="Metas")
@RestController
public class MetasController extends Controller {

	/**
	* Logger for this class
	*/
	private static final Logger logger = LoggerFactory.getLogger(MetasController.class.getName());



	public void index() {
		render("/page/metas/metasList.html");
	}

	@ApiOperation("GITHUB项目列表")
	@RequestMapping(value="/metas/show",method= RequestMethod.GET)
	@ResponseBody
	public Object show(//@ApiParam("项目名称") @RequestParam(value="pjname",defaultValue="",required=false) String pjname,
		...
		sql_exp.append("from t_metas a where 1=1 ");
...
//		sql_exp.append("order by a.UPDATE desc");
		...
		Page<Metas> pages= Metas.dao.paginate(pageNum,pageSize,sql_select.toString(),sql_exp.toString(),params.toArray());
