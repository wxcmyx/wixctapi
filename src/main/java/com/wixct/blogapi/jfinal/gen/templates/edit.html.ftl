...
<div class="jarviswidget " id="wid-id-4" data-widget-editbutton="false" data-widget-custombutton="false">
    ...
    <div>
        ...
        <form action="/${modelName?lower_case}/update" class="smart-form" method="post" id="${modelName?lower_case}Form">
        	<header>基础</header>
        	<fieldset>
        		<div class="row"><#list table.columns as it><#if it.columnName!='ID'>
					...
					<input type="hidden" name="${modelName?lower_case}.${it.columnName}" value="\$\{${modelName?lower_case}.${it.columnName}\!\}"/></#if><#if it_index%3==0>
				</div>
				<div class="row"></#if></#list>
				</div>
			</fieldset>
			<footer>
